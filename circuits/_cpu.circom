...
template Operand(bits) {
    ...
    var bitsPow = 2 ** bits;
    assert(raw < bitsPow);
    // neg <== (raw \ 2 ** (bits - 1)) * signed; // (raw >> 31) * signed
    neg <== (2 * raw \ bitsPow) * signed; // (raw >> 31) * signed
    component mux = Mux1();
    mux.c[0] <== raw;
    mux.c[1] <== bitsPow - raw;
    mux.s <== neg;
...
template Operate2(bits) {
    ...
    component lessThanU = cLessThan(bits);
    lessThanU.a <== op1Abs;
    lessThanU.b <== op2Abs;
    ...
    signal lt;
    lt <== diffSign * op1Neg + (1 - diffSign) * (ltU + op1Neg - 2 * ltU * op1Neg);

    mux4.c[0][0] <== op1Abs + op2Abs; // ADD
    mux4.c[1][0] <== 0; // ADD NEG
    mux4.c[0][1] <== (1 - diffSign) * (op1Abs - op2Abs) + diffSign * (op1Abs + op2Abs); // SUB
    mux4.c[1][1] <== lt; // SUB NEG
    ...
    for (var ii = 2; ii < 2 ** functBits; ii++) {
        mux4.c[0][ii] <== 0;
        mux4.c[1][ii] <== 0;
    }
